...
# Compartment onde os Jobs serão criados (OCR e LLM)
COMPARTMENT_ID = "ocid1.compartment.oc1..aaaaaaaa3yuok..................................a" 
...
object_location.object_name = "RG_test.jpeg" 
...
output_location.prefix = "rg_destino" 
...
# Extrai todo o texto do RG e junta numa única string
texto_bruto_linhas = []
...
# Instruções otimizadas para o Llama extrair os dados do RG
prompt_llama = f"""
Você é um assistente especializado em extração estruturada de dados de documentos de identidade brasileiros.
Abaixo está o texto bruto extraído de uma Carteira de Identidade (RG) através de OCR.
Sua tarefa é encontrar as informações pertinentes, formatar os nomes em formato de Título (Capitalizado) e retornar um ficheiro JSON.

As chaves do seu JSON DEVEM ser EXATAMENTE estas:
- "ORGAO_EXPEDIDOR"
- "REGISTRO_GERAL"
- "NOME"
- "FILIACAO" (deve ser um array/lista de strings contendo os nomes dos pais)
- "DATA_DE_EXPEDICAO"
...
4. Se uma informação não for encontrada, preencha o valor como "não encontrado".
...
chat_details = oci.generative_ai_inference.models.ChatDetails(
    ...
    chat_request=oci.generative_ai_inference.models.GenericChatRequest(
        ...
        max_tokens=400, 
        temperature=0.0, # Temperatura 0 para garantir respostas determinísticas e fiéis ao documento
        top_p=0.75
    )
)
...
try:
    ...
    if match:
        ...
        # Exibe o padrão definido no console
        print("\n--- JSON FINAL EXTRAÍDO PELO LLAMA (PADRÃO RG) ---")
        ...
        # -------------------------------------------------------------------------
        nome_arquivo_final = "resultado_final_rg.json" # Ficará na raiz do bucket
        
        print(f"\nA guardar o resultado final na raiz do bucket: {output_location.bucket_name} ...")
        ...
        print(f"Ficheiro '{nome_arquivo_final}' guardado com sucesso na raiz do bucket!")
