...
# Compartment onde os Jobs serão criados (tanto de OCR quanto de LLM)
COMPARTMENT_ID = "ocid1.compartment.oc1..aaaaaaaa3yuokzmsm34n..............................3a" 
...
# Para o LLM, precisamos apenas do texto bruto extraído
...
object_location.object_name = "CNH-e.pdf" # Ajustado para o formato do seu arquivo
...
output_location.prefix = "cnh_destino" 
...
# Extrai todo o texto da CNH e junta em uma única string
texto_bruto_linhas = []
...
# Instruções otimizadas para o Llama (REMOVIDO O PEDIDO DE RESUMO DE PALAVRAS)
prompt_llama = f"""
Você é um assistente especializado em extração estruturada de dados de documentos de identidade brasileiros.
Abaixo está o texto bruto extraído de uma CNH através de OCR.
Sua tarefa é encontrar as informações pertinentes, formatar os nomes em formato de Título (Capitalizado) e retornar um arquivo JSON.
...
- "VALIDADE"
- "Nº REGISTRO"
- "FILIAÇÃO" (deve ser um array/lista de strings contendo os nomes dos pais)
- "CAT HAB"

Valores de exemplo:
- "CAT HAB" - Só pode ser categoria "A", "B", "AB", "C", "D" ou "E". Encontre o valor correspondente senão o valor deve ser "não encontrado"
- "NOME" - deve ser constituído de letras somente.
...
chat_details = oci.generative_ai_inference.models.ChatDetails(
    ...
    chat_request=oci.generative_ai_inference.models.GenericChatRequest(
        ...
        max_tokens=400, # Reduzido, pois não geraremos mais o array gigante de palavras
        temperature=0.0,
        top_p=0.75
    )
)
...
try:
    ...
    if match:
        ...
        # Exibe o padrão definido limpo
        print("\n--- JSON FINAL EXTRAÍDO PELO LLAMA (PADRÃO CNH) ---")
        print(json.dumps(json_extraido_final, ensure_ascii=False, indent=4))
        print("---------------------------------------------------")
        
        # -------------------------------------------------------------------------
        # [NOVO PASSO] Upload do JSON Limpo para a RAIZ do Bucket Destino
        # -------------------------------------------------------------------------
        nome_arquivo_final = "resultado_final_cnh.json" # Ficará na raiz do bucket
